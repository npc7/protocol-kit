...
class EthersAdapter {
    ...
    async getStorageAt(address, position) {
        const content = await __classPrivateFieldGet(this, _EthersAdapter_provider, "f").getStorage(address, position);
    ...
    async signTypedData(safeEIP712Args) {
        ...
        if ((0, utils_2.isTypedDataSigner)(__classPrivateFieldGet(this, _EthersAdapter_signer, "f"))) {
            ...
            const signature = await __classPrivateFieldGet(this, _EthersAdapter_signer, "f").signTypedData(typedData.domain, typedData.primaryType === 'SafeMessage'
    ...
    encodeParameters(types, values) {
        return new (__classPrivateFieldGet(this, _EthersAdapter_ethers, "f").AbiCoder)().encode(types, values);
    }
    decodeParameters(types, values) {
        return new (__classPrivateFieldGet(this, _EthersAdapter_ethers, "f").AbiCoder)().decode(types, values);
    }
}
